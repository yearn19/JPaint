...
public class Circle extends AbstractShape {
    ...
    public boolean intersects(int xx,int yy,int ww, int hh){
        ...
        Ellipse2D.Double temp = new Ellipse2D.Double(xMin, yMin, Math.max(Math.abs(x1 - x2), Math.abs(y1 - y2)), Math.max(Math.abs(x1 - x2), Math.abs(y1 - y2)));
    ...
    public void fill(Graphics2D g2d) {
        ...
        g2d.setStroke(new BasicStroke(width));
        g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        g2d.fillOval(Math.min(x1, x2) + 1, Math.min(y1, y2) + 1, Math.max(Math.abs(x1 - x2), Math.abs(y1 - y2)) - 2,
                Math.max(Math.abs(x1 - x2), Math.abs(y1 - y2)) - 2);
    }
}